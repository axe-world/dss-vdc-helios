...
const char *g_cfgfile = "helios.cfg";
const char *version = "0.0.1";
int g_shutdown_flag = 0;
helios_data_t helios;
helios_vdcd_t* kwl_device = NULL;
    ...
    if (now >= last + 10) {
      ...
      if ( (g_query_values_time <= now)) {        
        ...
        if (rc == 0) {                 //getting values from KLAFS API succeeded and some values have changed compared to previous get values
          ...
          vdc_report(LOG_DEBUG, "KWL values did not change - not sending to DSS\n");
        } else {                                     //getting values from KLAFS API failed - retry in one minute
          now = time(NULL);
          g_query_values_time = 60 + now;
          g_network_changes = false;                 // no send to upstream DSS  
          dsvdc_send_pong(handle, kwl_device->dsuidstring);
        }
      }
      last = now;
    }
...
void push_sensor_data() {
  ...
  while (1) {
    if (kwl_device->kwl->sensor_values[i].is_active) {
      ...
      dsvdc_property_add_int (prop, "age", now - kwl_device->kwl->sensor_values[i].last_query);
      ...
      kwl_device->kwl->sensor_values[i].last_reported = now;
  ...
  dsvdc_push_property (handle, kwl_device->dsuidstring, pushEnvelope);
...
int main(int argc __attribute__((unused)), char **argv __attribute__((unused))) {
  ...
  memset(&helios, 0, sizeof(helios_data_t));
  int rc = read_config();
  if (rc < -1) {
  ...
  /* store configuration data, including the Helios device setup and the VDC DSUID */
  if (write_config() < 0) {
  ...
  while (!g_shutdown_flag) {
    ...
    if (!kwl_device->announced) {
      announce_device();
      continue;
    }

    if (!kwl_device->present) {
      if(kwl_device->presentSignaled) {
        dsvdc_device_vanished(handle, kwl_device->dsuidstring);
        kwl_device->presentSignaled = false;
        continue;
      }
    } else {
      if (!kwl_device->presentSignaled) {
        dsvdc_identify_device(handle, kwl_device->dsuidstring);
        kwl_device->presentSignaled = true;
        continue;
      } 
    }
    ...
    if (g_network_changes) {
      ...
      vdc_report(LOG_DEBUG, "Main loop: kwl_device %p: - dsuid %s - presentSignaled %s, announced %s\n",
            kwl_device, kwl_device->dsuidstring,
            kwl_device->presentSignaled ? "yes" : "no",
            kwl_device->announced? "yes" : "no"); 
